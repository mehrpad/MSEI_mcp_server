...
| Users can't connect | Firewall/subnet or wrong URL | From a user machine open `http://<VM-IP>:8080/health` in a browser. Re-check [step 06 §3](06-run-mcp-server.md#3-lock-down-the-network-important). |
| **Nobody** can reach `:8080` (even allowed PCs) | A `DOCKER-USER` `DROP` rule with no `RETURN` (allow) above it — blocks everyone | `iptables -S DOCKER-USER`; add the allow for your users' subnet: `iptables -I DOCKER-USER -p tcp --dport 8080 -s <users-subnet>/24 -j RETURN`, then `netfilter-persistent save`. The allowed subnet is the **users' PCs**, not the VM's subnet. |
